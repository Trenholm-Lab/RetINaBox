...
class DataManager(QObject):
    ...
    def __init__(self):
        ...
        self.neuron_connectivity_data = {
            "rgc1": {"polarity": "ON",
                      "connectivity": self.set_default_neuron_connectivity(),
                      "threshold": 1},
            "rgc2": {"polarity": "ON",
                      "connectivity": self.set_default_neuron_connectivity(),
                      "threshold": 1}
    ...
    """
    def set_neuronal_polarity(self, neuron_num, polarity):
        self.neuron_connectivity_data[f"rgc{neuron_num}"]["polarity"] = polarity
        self.neuron_connectivity_updated.emit()

    def get_neuron_polarity(self, neuron_num):
        """Get neuron polarity based on current mode (experiment or lab)"""
        if hasattr(self, 'data_mode') and self.data_mode == "experiment":
            return self.DM_connectivity_data[f"rgc{neuron_num}"]["polarity"]
        else:
            return self.neuron_connectivity_data[f"rgc{neuron_num}"]["polarity"]
    ...
    def set_all_connectivity(self, activity_data_dict):
        """Set all neurons to desired connectivity, threshold, and polarity"""
        # Reset connectivity for both RGC neurons
        for neuron_num in [1, 2]:
            # Reset connectivity to default (all off, no delays)
            self.neuron_connectivity_data[f"rgc{neuron_num}"]["connectivity"] = activity_data_dict[f"rgc{neuron_num}"]["connectivity"]
            # Reset threshold to default value
            self.neuron_connectivity_data[f"rgc{neuron_num}"]["threshold"] = activity_data_dict[f"rgc{neuron_num}"]["threshold"]
            self.neuron_connectivity_data[f"rgc{neuron_num}"]["polarity"] = activity_data_dict[f"rgc{neuron_num}"]["polarity"]
    ...
    def reset_all_connectivity(self):
        ...
        for neuron_num in [1, 2]:
            ...
            self.neuron_connectivity_data[f"rgc{neuron_num}"]["polarity"] = "ON"
    ...
    def load_challenge_connectivity(self, connectivity_array, threshold):
        ...
        self.DM_connectivity_data["rgc1"]["polarity"] = "ON"
        ...
        self.DM_connectivity_data["rgc2"]["polarity"] = "ON"
