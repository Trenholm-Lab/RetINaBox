...
class VisualStimManager(QGroupBox):
    def __init__(self, data_manager):
        ...
        self.data_manager.mode_changed.connect(self.on_mode_changed)  
    ...
    def update_visual_LED_grid(self, toggle_bool):
        ...
        if toggle_bool:
            ...
            self.visual_LED_grid.select_all_btn.setEnabled(False)
            self.visual_LED_grid.clear_all_btn.setEnabled(False)
        else: 
            self.mode_widget.setEnabled(True)
            self.visual_LED_grid.select_all_btn.setEnabled(True)
            self.visual_LED_grid.clear_all_btn.setEnabled(True)
    ...
    def on_mode_changed(self, mode):
        ...
        if mode == "experiment":
            ...
            self.visual_LED_grid.select_all_btn.setEnabled(True)
            self.visual_LED_gridclear_all_btn.setEnabled(True)
...
class LEDGrid(QWidget): # good yellow color =  #C49102;
    def __init__(self, data_manager):
        ...
        self.select_all_btn.setStyleSheet(BUTTON_STYLES['secondary'] + "font-size: 12px;") 
        self.select_all_btn.clicked.connect(self.select_all_leds)
        self.clear_all_btn = QPushButton("Clear all")
        self.clear_all_btn.setStyleSheet(BUTTON_STYLES['secondary'] + "font-size: 12px;")
        self.clear_all_btn.clicked.connect(self.clear_all_leds)

        selection_buttons_layout.addWidget(self.select_all_btn)
        selection_buttons_layout.addWidget(self.clear_all_btn)
...
class LEDControlButton(QPushButton):    
    def __init__(self, data_manager): 
        ...
        self.setText("Activate stimulus LEDs")
    ...
    def handle_click(self):
        ...
        self.data_manager.set_stim_running(state)
        if state:
            self.setText("Deactivate stimulus LEDs")
        else:
            self.setText("Activate stimulus LEDs")
...
class ConnectivityManager(QGroupBox):
    def __init__(self, data_manager):
        ...
        custom_style = GROUP_BOX_STYLES['default'] 
        ...
        self.thresholdWidget_1 = RGCThresholdWidget(neuron_num=1, data_manager=self.data_manager)
        self.polarityWidget_1 = RGCPolarityWidget(neuron_num=1, data_manager=self.data_manager)

        interactive_layout.addWidget(self.RGC_1, alignment=Qt.AlignCenter)
        
        # Create container for RGC1 controls
        rgc1_controls = QWidget()
        rgc1_controls.setStyleSheet("background: #F2F2F2;")
        rgc1_controls_layout = QVBoxLayout()
        rgc1_controls_layout.setContentsMargins(0,0,0,0)  # No margins
        rgc1_controls_layout.setSpacing(0)  # Minimal spacing between threshold and polarity
        
        rgc1_controls_layout.addWidget(self.thresholdWidget_1, alignment=Qt.AlignLeft)
        rgc1_controls_layout.addWidget(self.polarityWidget_1, alignment=Qt.AlignLeft)
        rgc1_controls.setLayout(rgc1_controls_layout)
        
        interactive_layout.addWidget(rgc1_controls)
        ...
        self.thresholdWidget_2 = RGCThresholdWidget(neuron_num=2, data_manager=self.data_manager)
        self.polarityWidget_2 = RGCPolarityWidget(neuron_num=2, data_manager=self.data_manager)
        interactive_layout.addWidget(self.RGC_2, alignment=Qt.AlignCenter)
        
        # Create container for RGC2 controls
        rgc2_controls = QWidget()
        rgc2_controls.setStyleSheet("background: #F2F2F2;")
        rgc2_controls_layout = QVBoxLayout()
        rgc2_controls_layout.setContentsMargins(0,0,0,0)  # No margins
        rgc2_controls_layout.setSpacing(0)  # Minimal spacing between threshold and polarity
        
        rgc2_controls_layout.addWidget(self.thresholdWidget_2, alignment=Qt.AlignLeft)
        rgc2_controls_layout.addWidget(self.polarityWidget_2)
        rgc2_controls.setLayout(rgc2_controls_layout)
        
        interactive_layout.addWidget(rgc2_controls)
        ...
        # (After both threshold widgets are created)
        self.data_manager.neuron_connectivity_updated.connect(self.update_from_data)
    ...
    def update_from_data(self):
        ...
        # Use the widgets' update methods to handle disconnection/reconnection
        self.thresholdWidget_1.update_from_data_manager()
        self.thresholdWidget_2.update_from_data_manager()
        self.polarityWidget_1.update_from_data_manager()
        self.polarityWidget_2.update_from_data_manager()

class RGCThresholdWidget(QWidget):
    def __init__(self, neuron_num, data_manager):
        ...
        # Create threshold label with same styling as polarity label
        threshold_label = QLabel("Threshold:")
        threshold_label.setStyleSheet("background: transparent; font-size: 13px; margin: 0; padding: 0;")
        threshold_label.setFixedWidth(75)  # Same width as polarity label
        
        # Create threshold spinbox
        self.threshold_spinbox = QSpinBox()
        self.threshold_spinbox.setRange(0, 9)
        self.threshold_spinbox.setSingleStep(1)
        self.threshold_spinbox.setValue(self.data_manager.get_neuron_threshold(self.neuron_num))
        self.threshold_spinbox.setFixedWidth(45)
        self.threshold_spinbox.setAlignment(Qt.AlignCenter)
        self.threshold_spinbox.setStyleSheet("margin: 0; padding: 0;")
        
        layout.addWidget(threshold_label, alignment=Qt.AlignLeft)
        layout.addWidget(self.threshold_spinbox, alignment=Qt.AlignLeft)
        layout.addStretch()  # Push everything to the left, matching polarity layout
    ...
    def on_threshold_changed(self, value):
        ...
        self.data_manager.set_neuron_threshold(self.neuron_num, value)

    def update_from_data_manager(self):
        """Update spinbox value from data manager (used when presets are loaded)"""
        # Temporarily disconnect to avoid triggering data updates
        try:
            self.threshold_spinbox.valueChanged.disconnect()
        except TypeError:
            pass  # No connections to disconnect
        
        # Update value from data manager
        self.threshold_spinbox.setValue(self.data_manager.get_neuron_threshold(self.neuron_num))
...
class RGCPolarityWidget(QWidget):
    def __init__(self, neuron_num, data_manager):
        super().__init__()
        self.data_manager = data_manager
        self.neuron_num = neuron_num 
        ...
        # Create polarity label with same styling as threshold label
        polarity_label = QLabel("Type:")
        polarity_label.setStyleSheet("background: transparent; font-size: 13px; margin: 0; padding: 0;")
        polarity_label.setFixedWidth(75)  # Same width as threshold label
        
        layout.addWidget(polarity_label, alignment=Qt.AlignLeft)
        self.radio_ON = QRadioButton("ON")
        self.radio_OFF = QRadioButton("OFF")
        # Set font size for radio buttons to match VisualStimManager style
        self.radio_ON.setStyleSheet("font-size: 13px;")
        self.radio_OFF.setStyleSheet("font-size: 13px;")
        
        layout.addWidget(self.radio_ON, alignment=Qt.AlignLeft)
        # Add some horizontal spacing between ON and OFF buttons
        layout.addSpacing(15)
        layout.addWidget(self.radio_OFF, alignment=Qt.AlignLeft)
        layout.addStretch()  # Push everything to the left, matching threshold layout
        
        # Set widget margins to zero
        self.setContentsMargins(0, 0, 0, 0)
        self.setLayout(layout)

        # Connect signals with checked parameter to avoid double-firing
        self.radio_ON.toggled.connect(self.on_polarity_changed)
        self.radio_OFF.toggled.connect(self.on_polarity_changed)
        
        # Initialize from data manager's current state for this neuron
        current_polarity = self.data_manager.neuron_connectivity_data[f"rgc{self.neuron_num}"]["polarity"]
        if current_polarity == "ON":
            self.radio_ON.setChecked(True)
        else:
            self.radio_OFF.setChecked(True)

    def on_polarity_changed(self, checked):    
        # Only respond when a button is being checked (not unchecked)
        if not checked:
            return
            
        if self.radio_ON.isChecked():
            self.data_manager.set_neuronal_polarity(self.neuron_num, "ON")
        elif self.radio_OFF.isChecked():
            self.data_manager.set_neuronal_polarity(self.neuron_num, "OFF")
    
    def update_from_data_manager(self):
        """Update polarity radio value from data manager (used when presets are loaded)"""
        # Temporarily disconnect to avoid triggering data updates
        try:
            self.radio_ON.toggled.disconnect()
            self.radio_OFF.toggled.disconnect()
        except TypeError:
            pass  # No connections to disconnect
...
class NeuronWidget(QWidget):
    ...
    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        # Use absolute path for RGC SVG
        script_dir = os.path.dirname(os.path.abspath(__file__))
        renderer = QSvgRenderer(os.path.join(script_dir, "Graphics", f"RGC_{self.neuron_num}.svg"))
        bounds = QRectF(self.rect())
