...
class LiveGrapherWidget(QGroupBox):
    ...
    def create_unified_legend(self, layout):
        ...
        legend_items = [
            {"color": COLORS['red'], "width": 3, "label": "Excitatory"},
            {"color": COLORS['blue'], "width": 3, "label": "Inhibitory"}, 
        ]
    ...
    def create_rgc_groupboxes(self, layout):
        ...
        # Add the scale bar to plot3
        self.add_scale_bar_to_plot(self.plot3)
        
        # Create time label widget
        time_label_widget = QWidget()
        time_label_widget.setStyleSheet("background: transparent;")
        time_label_layout = QHBoxLayout()
        time_label_layout.setContentsMargins(0, 0, 0, 0)
        time_label_layout.setSpacing(5)
        
        time_label_layout.addStretch(4)
        
        # "Time" label
        time_text = QLabel("Time")
        time_text.setFont(pg.QtGui.QFont(FONTS['family'], 10))
        time_text.setAlignment(Qt.AlignCenter)
        time_text.setStyleSheet("background: transparent;")
        time_label_layout.addWidget(time_text)
        
        # Stretch between "Time" and "1 sec"
        time_label_layout.addStretch(3)
        
        # Right-aligned "1 sec" label
        sec_text = QLabel("1 sec")
        sec_text.setFont(pg.QtGui.QFont(FONTS['family'], 9))
        sec_text.setAlignment(Qt.AlignRight)
        sec_text.setStyleSheet("background: transparent;")
        time_label_layout.addWidget(sec_text)
        
        # Small stretch at the end to align with scale bar
        time_label_layout.addStretch(1)
        
        time_label_widget.setLayout(time_label_layout)
        rgc1_layout.addWidget(time_label_widget)
        ...
        # Add the scale bar to plot4
        self.add_scale_bar_to_plot(self.plot4)
        ...
        time_label_widget.setLayout(time_label_layout)
        rgc2_layout.addWidget(time_label_widget)
    ...
    def setup_plots(self):
        ...
        for _ in range(9):
            ...
            rgc1_curves = {
                'gray': self.plot1.plot(pen=pg.mkPen(color=COLORS['gray'], width=0.5, alpha=0.3)),
                'red': self.plot1.plot(pen=pg.mkPen(color=COLORS['red'], width=4, alpha=0.8)),
                'blue': self.plot1.plot(pen=pg.mkPen(color=COLORS['blue'], width=4, alpha=0.8))
            }
            rgc2_curves = {
                'gray': self.plot2.plot(pen=pg.mkPen(color=COLORS['gray'], width=0.5, alpha=0.3)),
                'red': self.plot2.plot(pen=pg.mkPen(color=COLORS['red'], width=4, alpha=0.8)),
                'blue': self.plot2.plot(pen=pg.mkPen(color=COLORS['blue'], width=4, alpha=0.8))
            }
        ...
        self.rgc1_curves = {
            'gray': self.plot3.plot(pen=pg.mkPen(color=COLORS['gray'], width=0.5, alpha=0.3)),
        ...
        self.rgc2_curves = {
            'gray': self.plot4.plot(pen=pg.mkPen(color=COLORS['gray'], width=0.5, alpha=0.3)),
        ...
        # Configure bottom axis for plots with ticks above the axis line
        for plot in [self.plot1, self.plot2, self.plot3, self.plot4]:
            plot.getAxis('bottom').setTicks(x_ticks)
            # Configure ticks to appear above the axis
            plot.getAxis('bottom').setStyle(
                tickTextOffset=0,
                tickLength=-5,  # Negative value makes ticks point upward
                tickTextHeight=0  # No text height since we're not showing labels
            )
        ...
        photoreceptor_label_1.setColor(COLORS['black'])  # Match y-axis label color
        ...
        photoreceptor_label_2.setColor(COLORS['black'])  # Match y-axis label color
        photoreceptor_label_2.setPos(425, 8)  
        self.plot2.addItem(photoreceptor_label_2)

    def add_scale_bar_to_plot(self, plot_widget):
        """Add a scale bar directly to the plot"""
        # Create scale bar (1 second = 40 timepoints)
        scale_bar = pg.PlotDataItem(
            x=[360, 400],  # 9s to 10s position
            y=[-0.4, -0.4],  # Position near bottom
            pen=pg.mkPen(color=COLORS['black'], width=1)
        )
        plot_widget.addItem(scale_bar)
        
        # Add tick marks
        left_tick = pg.PlotDataItem(
            x=[360, 360],
            y=[-0.45, -0.35],
            pen=pg.mkPen(color=COLORS['black'], width=1)
        )
        right_tick = pg.PlotDataItem(
            x=[400, 400],
            y=[-0.45, -0.35],
            pen=pg.mkPen(color=COLORS['black'], width=1)
        )
        plot_widget.addItem(left_tick)
        plot_widget.addItem(right_tick)
