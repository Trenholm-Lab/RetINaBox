...
def _check_custom_presets():
    ...
    custom_presets_dir = os.path.join(project_root, 'custom_presets')
    
    if os.path.exists(custom_presets_dir):
        pass
    else:
        # create the directory if it does not exist
        os.makedirs(custom_presets_dir)
...
class PresetOpenButton(QToolButton):
    ...
    def __init__(self, data_manager):
        super().__init__()
        self.custom_presets_dir = _check_custom_presets()
    ...
    def open_preset_file_dialog(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            self.custom_presets_dir,  # Use absolute path to custom_presets
            "JSON Files (*.json)"
        )
