...
from interaction_manager import RGCThresholdWidget, RGCPolarityWidget
from styles import *
import os 
import json
...
class UserNeuronWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        renderer = QSvgRenderer(os.path.join(script_dir, "Graphics", "RGC_M.svg"))
...
class DiscoveryModeWidget(QWidget):
    ...
    def create_challenge_controls_panel(self):
        ...
        challenge_items = []
        script_dir = os.path.dirname(os.path.abspath(__file__))
        json_path = os.path.join(script_dir, ".default_code", ".default_discoveryMode_challenges.json")
    ...
    def reset_connectivity_panel(self):
        ...
        # Reset threshold and polarity to default values
        self.user_threshold_widget.threshold_spinbox.setValue(1)
        self.user_polarity_widget.radio_ON.setChecked(True)  # Set to ON by default
    ...
    def create_connectivity_panel(self):
        ...
        # Threshold and Polarity controls
        controls_container = QWidget()
        controls_layout = QHBoxLayout()
        controls_layout.setContentsMargins(5, 5, 5, 5)
        controls_layout.setSpacing(10)
        
        # Create RGCThresholdWidget
        self.user_threshold_widget = RGCThresholdWidget(1, self.data_manager)  # Using neuron 1 for discovery mode
        controls_layout.addWidget(self.user_threshold_widget)
        
        # Create RGCPolarityWidget
        self.user_polarity_widget = RGCPolarityWidget(1, self.data_manager)  # Using neuron 1 for discovery mode
        controls_layout.addWidget(self.user_polarity_widget)
    ...
    def create_bottom_rgc_panel(self):
        ...
        self.rgc_plot.getAxis('left').setPen(pg.mkPen(color=COLORS['black'], width=2))
        self.rgc_plot.getAxis('bottom').setPen(pg.mkPen(color=COLORS['black'], width=2))
        self.rgc_plot.getAxis('left').setTextPen(pg.mkPen(color=COLORS['black']))
        self.rgc_plot.getAxis('bottom').setTextPen(pg.mkPen(color=COLORS['black']))
        ...
        self.rgc_curves = {
            'gray': self.rgc_plot.plot(pen=pg.mkPen(color=COLORS['gray'], width=0.5, alpha=0.3)),
            'yellow': self.rgc_plot.plot(pen=pg.mkPen(color=COLORS['yellow'], width=8, alpha=0.9))
        }
    ...
    def test_connectivity(self):
        ...
        # Get threshold and polarity from widgets
        user_threshold = self.user_threshold_widget.threshold_spinbox.value()
        user_polarity = "ON" if self.user_polarity_widget.radio_ON.isChecked() else "OFF"
        ...
        if isinstance(challenge_data, list):
            ...
            # go through each solution and see if it matches phase 1 and phase 2 answer
            user_stim = np.array(self.data_manager.DM_stim).astype(bool) 
            user_motion = self.motion_combo.currentText().lower()
            user_dir = self.direction_combo.currentText().lower()
            user_speed = self.speed_combo.currentText().lower()

            for key_index in range(len(challenge_data)):
                target_solution = challenge_data[key_index]
                key_stim = np.array(target_solution['stim_array']).astype(bool)
                key_motion = target_solution['motion_type'].lower()
                key_dir = "---" if target_solution['direction'] == "" else target_solution['direction'].lower()
                key_speed = "---" if target_solution['speed'] == "" else target_solution['speed'].lower()
                key_connectivity = np.array(target_solution['connectivity'])
                key_threshold = target_solution['threshold']
                key_polarity = target_solution.get('polarity', 'ON')  # Default to ON if not specified
                
                if (np.array_equal(key_connectivity, user_connectivity) and 
                    key_threshold == user_threshold and 
                    key_polarity == user_polarity) and (np.array_equal(user_stim, key_stim) and 
                    user_motion == key_motion and 
                    user_dir == key_dir and 
                    user_speed == key_speed):
            ...
            key_polarity = challenge_data.get('polarity', 'ON')  # Default to ON if not specified
            if (np.array_equal(key_connectivity, user_connectivity) and 
                key_threshold == user_threshold and 
                key_polarity == user_polarity):
                connectivity_match = True
    ...
    def set_connectivity_panel_visual_state(self, enabled):
        ...
        if enabled:
            ...
            # self.threshold_widget.setStyleSheet(LEGEND_STYLES['discovery_legend'])
            ...
            #self.threshold_widget.setStyleSheet(grayed_style)
