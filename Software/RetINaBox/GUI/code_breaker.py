...
class CypherWidget(QGroupBox):
    def __init__(self, RGC_1_pref_stim, RGC_2_pref_stim, RetinaBox_output):
        ...
        self.setLayout(self.layout)
        # Remove fixed minimum width to allow stretching
        ...
        # Add stretch above to push content down from top
        self.layout.addStretch(1)
        self.layout.addWidget(pref_visual_stim_box)
        
        # Add stretch between boxes to create space
        #self.layout.addStretch(1)
        ...
        # Calculate exact height needed: header (30px) + 4 rows * 30px
        table.setFixedHeight(150)  # 30px header + (4 * 30px rows)
        table.setFixedWidth(215)  # Set fixed width to match column widths
        table.setHorizontalHeaderLabels(['RGC1', 'RGC2', 'Letter'])
        table.verticalHeader().setVisible(False)
        table.horizontalHeader().setFixedHeight(30)  # Set header height
    
        table.setStyleSheet(TABLE_STYLES['cypher'])
        
        for i in range(4):
            table.setRowHeight(i, 30)  # Set consistent row heights
        ...
        # Add the container to the output layout
        output_layout.addWidget(container_widget)  
...
class CodeBreakerWidget(QWidget):
    ...
    def build_challenge_content(self, challenge_data):
        ...
        code_container.setMinimumWidth(700)  # Minimum width instead of fixed
        code_container.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        main_layout.addWidget(code_container, stretch=7)  # Give code container more stretch

        # Create right side container for cypher and button
        ...
        right_layout.addWidget(cypher_widget)
        ...
        # Add right container to main layout with smaller stretch factor
