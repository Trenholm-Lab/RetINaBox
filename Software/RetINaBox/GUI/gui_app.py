...
class HomeWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        logo_svg = QSvgWidget(os.path.join(script_dir, "Graphics", "Logo_noText.svg"))
...
class RetinaLabWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setMinimumSize(QSize(1000, 750))  # Set minimum size instead of fixed
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.setWindowIcon(QIcon(os.path.join(script_dir, "Graphics", "Logo_noText.svg")))
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)  # Allow window to expand
    ...
    def create_main_lab_tab(self):
        ...
        # Set size policies for the manager widgets
        self.visual_stim_manager.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        self.connectivity_manager.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        self.live_graph_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)

        # Add widgets with appropriate stretch factors
        central_layout.addWidget(self.visual_stim_manager, stretch=1)  
        central_layout.addWidget(self.connectivity_manager, stretch=1)  
        central_layout.addWidget(self.live_graph_widget, stretch=3)  # Give more space to the graph
...
class LessonsButton(QToolButton):
    ...
    def load_activity_presets(self):
        ...
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            json_path = os.path.join(script_dir, ".default_code", ".default_activity_presets.json")
    ...
    def create_lesson_menus(self, parent_menu):
        ...
        for lesson_name, activities in self.activity_presets.items():
            # Create lesson submenu - preserve original case from preset file
            lesson_menu = QMenu(lesson_name.replace("_", " "), parent_menu)
    ...
    def load_activity_preset(self, lesson_name, activity_name):
        ...
        for rgc_key in ['rgc1', 'rgc2']:
            if rgc_key in activity_data_copy.keys():
                ...
                activity_data_copy[rgc_key] = {"connectivity": self.data_manager.set_default_neuron_connectivity(), "threshold": 1, "polarity": "ON"}
...
def open_lesson_plan(parent):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)  # Go up one level from GUI/
        abs_pdf_path = os.path.join(project_root, "Lesson_plan.pdf")
...
def open_manual(parent):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)  # Go up one level from GUI/
    abs_pdf_path = os.path.join(project_root, "User_manual.pdf")
    ...
    if os.path.exists(abs_pdf_path):
        ...
        msg.setInformativeText(f"Please place 'User_manual.pdf' in the project root folder:\n{project_root}")
