...
def install_pi400_system_stack():
    ...
    core_pkgs = [
        ...
        "python3-rpi.gpio",     # GPIO for Pi 1-4
        "python3-lgpio"         # GPIO for Pi 5 (newer alternative)
    ]
...
def setup_gpio_manager_files(model):
    """Select and rename the appropriate GPIO manager file based on Pi model."""
    print_header("Configuring GPIO Manager Files")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    gui_dir = os.path.join(script_dir, "GUI")
    
    # Define file paths
    lgpio_file = os.path.join(gui_dir, "gpio_manager.py")      # Pi 5 (lgpio)
    rpi_gpio_file = os.path.join(gui_dir, "gpio_manager_0.py") # Pi 1-4 (RPi.GPIO)
    active_file = os.path.join(gui_dir, "gpio_manager.py")
    backup_file = os.path.join(gui_dir, ".gpio_manager_backup.py")
    
    # Check if both files exist
    if not os.path.exists(lgpio_file):
        print(f"ERROR: {lgpio_file} not found!")
        sys.exit(1)
    if not os.path.exists(rpi_gpio_file):
        print(f"ERROR: {rpi_gpio_file} not found!")
        sys.exit(1)
    
    if model == "pi500":
        # Pi 5: use lgpio version (gpio_manager.py is already correct)
        # Backup the RPi.GPIO version
        if os.path.exists(rpi_gpio_file):
            shutil.copy2(rpi_gpio_file, backup_file)
            os.rename(rpi_gpio_file, os.path.join(gui_dir, ".gpio_manager_rpi.py"))
        print("Using lgpio GPIO manager (Pi 5 compatible)")
        print(f"RPi.GPIO version backed up as .gpio_manager_rpi.py")
        
    else:
        # Pi 1-4 or Pi 400: use RPi.GPIO version
        # First backup the current lgpio version
        if os.path.exists(lgpio_file):
            shutil.copy2(lgpio_file, os.path.join(gui_dir, ".gpio_manager_lgpio.py"))
        
        # Replace with RPi.GPIO version
        shutil.copy2(rpi_gpio_file, active_file)
        os.rename(rpi_gpio_file, backup_file)
        print("Using RPi.GPIO GPIO manager (Pi 1-4 compatible)")
        print(f"lgpio version backed up as .gpio_manager_lgpio.py")
        print(f"Original RPi.GPIO file backed up as .gpio_manager_backup.py")
...
def main():
    ...
    if model == "pi400":
        ...
        setup_gpio_manager_files(model)
        setup_application()
